...
public class SqlLiteBookRepositoryTests
{
    ...
    {
        _mockQueryExecutor.Setup(qe => qe.QueryAsync<int>(It.IsAny<string>(), It.IsAny<object>()))
            .ReturnsAsync(new List<int> { newId });

        await Sut.CreateBookAsync(book);

        _mockQueryExecutor.Verify(
            qe => qe.QueryAsync<int>(
                It.Is<string>(s =>
                    s.Contains("INSERT INTO books (Author, Title, Price) VALUES (@Author, @Title, @Price);")), book),
            Times.Once);
    }
    ...
    public async Task DeleteBook_CallsQueryExecutorWithDeleteString(int id)
    {
        ...
        _mockQueryExecutor.Verify(
            qe => qe.ExecuteAsync(It.Is<string>(s => s.Contains("DELETE FROM books WHERE Id = @id")),
                Capture.In(parameters)), Times.Once);
    ...
    {
        _mockQueryExecutor.Setup(qe => qe.QueryAsync<Book>(It.IsAny<string>(), It.IsAny<object>()))
        ...
        _mockQueryExecutor.Verify(
            qe => qe.QueryAsync<Book>(It.Is<string>(s => s.Contains("SELECT * FROM books WHERE id = @id")),
                Capture.In(parameters)), Times.Once);
    ...
    public async Task GetBooks_CallsQueryExecutorWithCorrectQuery_ReturnsBooks(List<Book> books)
    {
        ...
        _mockQueryExecutor.Verify(
            qe => qe.QueryAsync<Book>(It.Is<string>(s => s.Contains("SELECT * FROM books")), null), Times.Once);
    ...
    public async Task UpdateBook_CallsQueryExecutorWithCorrectQuery(int id, Book book)
    {
        ...
        _mockQueryExecutor.Verify(qe => qe.ExecuteAsync(
                It.Is<string>(s =>
                    s.Contains("UPDATE books SET Author = @Author, Title = @Title, Price = @Price WHERE Id = @Id")),
                It.Is<Book>(
                    b => b.Id == id && b.Price == book.Price && b.Author == book.Author && b.Title == book.Title)),
            Times.Once);
    }
