...
namespace BookStore.Api.UnitTests
{
    ...
    public class BookControllerTests
    {
        ...
        public void SetUp()
        {
            ...
            _mockValidator.Setup(v => v.ValidateAsync(It.IsAny<Book>(), It.IsAny<CancellationToken>())).ReturnsAsync(new ValidationResult());
        }
        ...
        public async Task BooksAsync_CallsService_ReturnsBooks(List<Book> books)
        {
            ...
            response.Result.Should().Match<OkObjectResult>(r => r.Value == books);
        }
        ...
        public async Task PostBook_ValidModel_CallsServiceAndReturnsNewId(Book book, int id)
        {
            ...
            response.Result.Should().Match<ObjectResult>(r => r.StatusCode == StatusCodes.Status201Created && r.Value.As<int>() == id);
        }

        [Test]
        [AutoData]
        public async Task PostBook_InvalidModel_ReturnsValidationErrors(Book book, int id, ValidationResult validationResult)
        {
            _mockValidator.Setup(v => v.ValidateAsync(It.IsAny<Book>(), It.IsAny<CancellationToken>())).ReturnsAsync(validationResult);
            _mockBookService.Setup(bs => bs.CreateBookAsync(book)).ReturnsAsync(id);
            ...
            response.Result.Should().Match<BadRequestObjectResult>(r => r.StatusCode == StatusCodes.Status400BadRequest && r.Value.As<string>() == validationMessage);
        }
        ...
        [Test]
        [AutoData]
        public async Task PutBookByIdAsync_InvalidModel_ReturnsValidationErrors(Book book, int id, ValidationResult validationResult)
        {
            _mockValidator.Setup(v => v.ValidateAsync(It.IsAny<Book>(), It.IsAny<CancellationToken>())).ReturnsAsync(validationResult);
            ...
            response.Should().Match<BadRequestObjectResult>(r => r.StatusCode == StatusCodes.Status400BadRequest && r.Value.As<string>() == validationMessage);
        }

        [Test]
        [AutoData]
        public async Task PutBookByIdAsync_ServiceThrowsException_PropagatesException(Book book, int id, Exception exception)
        {
        ...
        public async Task GetBookByIdAsync_ValidModel_CallsServiceAndReturnsBook(Book book, int id)
        {
            ...
            response.Result.Should().Match<OkObjectResult>(r => r.StatusCode == StatusCodes.Status200OK && r.Value.As<Book>() == book);
        }
        ...
        [Test]
        [AutoData]
        public async Task DeleteBookByIdAsyncl_CallsService(int id)
        {
